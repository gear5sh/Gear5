...
var RootCmd = &cobra.Command{
	Use:   "kaku",
	Short: "Kaku is a data pipeline connectors",
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if ok := utils.IsValidSubcommand(getAvailableCommands(), args[0]); !ok {
			return fmt.Errorf("'%s' is an invalid command. Use 'kaku --help' to display usage guide", args[0])
		}
...
func init() {
	...
	RootCmd.PersistentFlags().StringVarP(&config, "config", "", "", "(Required) Config for Kaku connector")
	RootCmd.PersistentFlags().StringVarP(&catalog, "catalog", "", "", "(Required) Catalog for Kaku connector")
	RootCmd.PersistentFlags().StringVarP(&state, "state", "", "", "(Required) State for Kaku connector")
	RootCmd.PersistentFlags().Int64VarP(&batchSize, "batch", "", 1000, "(Optional) Batch size for Kaku connector")
